...
class ProductRepositoryImpl implements ProductRepository {
  ...
  Future<Either<FailureException, ProductList>> fetchProductList() async {
    try {
      final response = await remoteDataSource.fetchProductList();
  ...
  @override
  Future<Either<FailureException, int>> deleteProduct(
      ProductTable product) async {
    try {
      final response = await localDataSource.deleteProduct(product);
      return Right(response);
    } catch (error) {
      return Left(DatabaseFailure(error.toString()));
    }
